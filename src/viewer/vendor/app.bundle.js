"use strict";
(() => {
  // ../lib/core/data-csrf.ts
  var TREE = __DATA__;
  var EMBED_CONTENT = __EMBED_CONTENT__;
  var EMBED_BACKLINKS = __EMBED_BACKLINKS__;
  var EMBED_NOTES = __EMBED_NOTES__;
  var EMBED_TASKS = __EMBED_TASKS__;
  var EMBED_ACTIVITY = __EMBED_ACTIVITY__;
  var DOC_TEMPLATES = __TEMPLATES__;
  var IS_OFFLINE_BUILD = EMBED_CONTENT !== null;
  var SITE_NAME = document.title;
  var TAGLINE = __TAGLINE_JSON__;
  var SITE_PREFIX = __SITE_PREFIX_JSON__;
  var csrfToken = null;
  var meState = null;
  function setMeState(v) {
    meState = v;
  }
  var totpEnabled = false;
  function setTotpEnabled(v) {
    totpEnabled = v;
  }
  ...
  // ../lib/core/sse-coord.ts
  var SseCoord = class {
    constructor() {
      this.selfSaveUntil = {};
      this.taskWrites = /* @__PURE__ */ new Set();
    }
  ...
  // ../lib/core/dom-refs.ts
  var treeEl = document.getElementById("tree");
  var contentEl = document.getElementById("content");
  var breadcrumbPath = document.getElementById("breadcrumb-path");
  var breadcrumbDate = document.getElementById("breadcrumb-date");
  var breadcrumbActions = document.getElementById("breadcrumb-actions");
  ...
  var tocNotes = document.getElementById("toc-notes");
  ...
  var STRINGS = {
    fr: {
      ...
      settingsRemoteFrom: (h2) => "depuis " + h2,
  ...
  // ../lib/core/state.ts
  var tocHasLinks = false;
  function setTocHasLinks(v) {
    tocHasLinks = v;
  }
  var tocHasNotes = false;
  function setTocHasNotes(v) {
    tocHasNotes = v;
  }
  var currentFile = null;
  function setCurrentFile(v) {
    currentFile = v;
  }
  var editMode = false;
  ...
  function setEditTextarea(v) {
    editTextarea = v;
  }
  var mdCount = 0;
  function setMdCount(v) {
    mdCount = v;
  }
  var otherCount = 0;
  function setOtherCount(v) {
    otherCount = v;
  }
  var EMBED_MIND = location.hash.replace(/^#/, "") === "mind";
  var isServerMode = (location.protocol === "http:" || location.protocol === "https:") && !IS_OFFLINE_BUILD;

  // ../lib/home/layout-chrome.ts
  var LayoutChrome = class {
    constructor() {
  ...
  var Combobox = class {
    ...
    compute() {
      const raw2 = this.input.value.trim();
      const q = raw2.toLowerCase();
      ...
      return { res, create: this.opts.creatable && raw2 && !exact ? raw2 : null };
    }
    render() {
  ...
  // ../lib/modals/dir-rename-modal.ts
  var DirRenameModal = class extends Modal {
    constructor() {
      super(dirRenameBackdrop);
      this.form = document.getElementById("dir-rename-form");
      this.input = document.getElementById("dir-rename-input");
      this.current = document.getElementById("dir-rename-current");
      this.error = document.getElementById("dir-rename-error");
      this.cancel = document.getElementById("dir-rename-cancel");
  ...
  // ../lib/admin/settings/settings-shared.ts
  var btnShare = document.getElementById("btn-share");
  var shareBackdrop = document.getElementById("share-backdrop");
  var sharePath = document.getElementById("share-path");
  var shareStep1 = document.getElementById("share-step1");
  var shareStep2 = document.getElementById("share-step2");
  ...
  var shareNew = document.getElementById("share-new");
  var shareExisting = document.getElementById("share-existing");
  var shareExistingList = document.getElementById("share-existing-list");
  var shareExistingCount = document.getElementById("share-existing-count");
  ...
  var _QrCode = class _QrCode {
    ...
    static gfMul(a, b) {
      return a === 0 || b === 0 ? 0 : _QrCode.exp[_QrCode.log[a] + _QrCode.log[b]];
    }
    // EC codewords for `data`: the remainder of the GF polynomial division by the
    // degree-ecLen generator.
    static rsEncode(data, ecLen) {
      const gen = _QrCode.genPoly(ecLen);
      const buf = data.concat(new Array(ecLen).fill(0));
      for (let i = 0; i < data.length; i++) {
        const coef = buf[i];
        if (coef === 0) continue;
        for (let j = 1; j < gen.length; j++) buf[i + j] ^= _QrCode.gfMul(gen[j], coef);
      }
    ...
    static genPoly(n) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < poly.length; j++) {
          next[j] ^= poly[j];
          next[j + 1] ^= _QrCode.gfMul(poly[j], _QrCode.exp[i]);
        }
        poly = next;
      }
      return poly;
    ...
    assembleCodewords(bytes, spec) {
      ...
      for (let bI = 0; bI < spec.blocks; bI++) {
        ...
        ecBlocks.push(_QrCode.rsEncode(chunk, spec.ecPerBlock));
      }
      const finalCw = [];
  ...
  _QrCode.exp = new Array(512);
  _QrCode.log = new Array(256);
  (() => {
    let x = 1;
    for (let i = 0; i < 255; i++) {
      _QrCode.exp[i] = x;
    ...
    for (let i = 255; i < 512; i++) _QrCode.exp[i] = _QrCode.exp[i - 255];
  })();
  ...
  // ../lib/admin/totp/totp-enroll-modal.ts
  var _TotpEnrollModal = class _TotpEnrollModal {
    constructor() {
      // a 6-digit code is a TOTP, anything else a recovery code
      this.backdrop = document.getElementById("totp-backdrop");
      this.modalTitle = document.getElementById("totp-title");
      this.errorBox = document.getElementById("totp-error");
      this.closeBtn = document.getElementById("totp-close");
      this.stepEnroll = document.getElementById("totp-step-enroll");
      this.stepRecovery = document.getElementById("totp-step-recovery");
      this.stepDisable = document.getElementById("totp-step-disable");
      this.qr = document.getElementById("totp-qr");
      this.secretValue = document.getElementById("totp-secret-value");
      this.secretCopy = document.getElementById("totp-secret-copy");
      this.verifyForm = document.getElementById("totp-verify-form");
      this.verifyCode = document.getElementById("totp-verify-code");
      this.verifySubmit = document.getElementById("totp-verify-submit");
      this.enrollCancel = document.getElementById("totp-enroll-cancel");
      this.recoveryList = document.getElementById("totp-recovery-list");
      this.recoveryCopy = document.getElementById("totp-recovery-copy");
      this.recoveryDone = document.getElementById("totp-recovery-done");
      this.disableForm = document.getElementById("totp-disable-form");
      this.disableCode = document.getElementById("totp-disable-code");
      this.disableSubmit = document.getElementById("totp-disable-submit");
      // ---- state ----
      this.recoveryCodes = [];
    ...
    async enroll() {
      try {
        ...
        settingsPanel.showError(err.message || t("settingsErrGeneric"));
      }
    }
  ...
  _TotpEnrollModal.SIX_DIGITS = /^[0-9]{6}$/;
  var TotpEnrollModal = _TotpEnrollModal;
  var totpModal = new TotpEnrollModal();

  // ../lib/admin/totp/security-pane.ts
  var SecurityPane = class {
    constructor() {
      this.statusBadge = document.getElementById("security-totp-status");
      this.enableBtn = document.getElementById("security-totp-enable");
      this.disableBtn = document.getElementById("security-totp-disable");
      this.logoutAllBtn = document.getElementById("security-logout-all");
      this.wire();
    }
  ...
  var securityPane = new SecurityPane();

  // ../lib/core/rng.ts
  var Rng = class _Rng {
    constructor(identity) {
      this.seed = _Rng.hash(identity);
      this.a = this.seed;
    }
    // Next value in [0, 1).
    next() {
      ...
      let t2 = Math.imul(this.a ^ this.a >>> 15, 1 | this.a);
  ...
  var _Avatar = class _Avatar {
    ...
    buildNodes() {
      ...
      for (let i = 0; i < satN; i++) {
        const t2 = even ? i / satN : i / satN + (r.next() - 0.5) * 0.16;
        const ang = baseAng + t2 * Math.PI * 2;
  ...
  var avatarSeed = Avatar.seed;

  // ../lib/modals/reset-password.ts
  var RESET_PW_MIN = 8;
  var resetPwBackdrop = document.getElementById("reset-pw-backdrop");
  var resetPwForm = document.getElementById("reset-pw-form");
  var resetPwEmail = document.getElementById("reset-pw-email");
  var resetPwInput = document.getElementById("reset-pw-input");
  var resetPwConfirm = document.getElementById("reset-pw-confirm");
  var resetPwToggle = document.getElementById("reset-pw-toggle");
  var resetPwEye = document.getElementById("reset-pw-eye");
  var resetPwEyeOff = document.getElementById("reset-pw-eye-off");
  var resetPwError = document.getElementById("reset-pw-error");
  var resetPwSuccess = document.getElementById("reset-pw-success");
  var resetPwSubmit = document.getElementById("reset-pw-submit");
  var resetPwCancel = document.getElementById("reset-pw-cancel");
  var resetPwClose = document.getElementById("reset-pw-close");
  ...
  var resetPwModal = new ResetPwModal();

  // ../lib/admin/settings/users-tab.ts
  var SettingsUsers = class {
    constructor(ctx) {
      this.ctx = ctx;
      this.list = document.getElementById("settings-users-list");
      this.form = document.getElementById("settings-user-form");
      this.inviteResult = document.getElementById("settings-invite-result");
      this.form.addEventListener("submit", (e) => this.submit(e));
  ...
  // ../lib/admin/settings/tokens-tab.ts
  var SettingsTokens = class {
    constructor(ctx) {
      this.ctx = ctx;
      this.list = document.getElementById("settings-tokens-list");
      this.form = document.getElementById("settings-token-form");
      this.result = document.getElementById("settings-token-result");
      this.form.addEventListener("submit", (e) => this.submit(e));
  ...
  // ../lib/admin/settings/shares-tab.ts
  var SettingsShares = class {
    constructor(ctx) {
      this.ctx = ctx;
      this.list = document.getElementById("settings-shares-list");
      this.list.addEventListener("click", (e) => this.onClick(e));
    }
    async load() {
      this.list.innerHTML = "";
  ...
  // ../lib/admin/settings/nodes-tab.ts
  var SettingsNodes = class {
    constructor(ctx) {
      this.ctx = ctx;
      this.list = document.getElementById("settings-nodes-list");
      this.form = document.getElementById("settings-node-form");
      this.result = document.getElementById("settings-node-result");
      this.form.addEventListener("submit", (e) => this.submit(e));
    ...
    async onClick(e) {
      ...
      if (relinkBtn) {
        const ok2 = await Dialogs.confirm({
  ...
  // ../lib/editor/wikilink-autocomplete.ts
  var _WikilinkAutocomplete = class _WikilinkAutocomplete {
    constructor() {
      this.isOpen = false;
      this.items = [];
      this.active = 0;
  ...
  _WikilinkAutocomplete.STYLE_KEYS = [
    "boxSizing",
    "width",
    "paddingTop",
    "paddingRight",
  ...
  var WikilinkAutocomplete = _WikilinkAutocomplete;

  // ../lib/content/tag-browse.ts
  var TagBrowsePage = class {
    showTag(tag) {
  ...
  var tagBrowsePage = new TagBrowsePage();
  ...
  var TagStore = class _TagStore {
    ...
    async persistTags(file, customTags) {
      ...
      const newContent = _TagStore.setFrontmatterTags(loaded, customTags);
      try {
        ...
        Dialogs.notifyError("tagSaveFailed", err.message);
        return false;
      }
  ...
  var _TagEditor = class _TagEditor {
    constructor(store) {
      this.store = store;
  ...
  _TagEditor.EDITOR_CLASS = "fixed z-50 w-64 bg-navy-800 border subtle-border rounded-lg shadow-2xl shadow-black/70 p-3";
  var TagEditor = _TagEditor;
  var docTags = new DocTags();
  var tagStore = new TagStore(docTags);
  var tagEditor = new TagEditor(tagStore);
  tagEditor.init();
  ...
  // ../lib/content/content-decorators.ts
  ...
  // ../lib/content/task-markers.ts
  var _TaskMarkers = class _TaskMarkers {
    ...
    static toggleNthTaskMarker(content, index2, checked) {
  ...
  _TaskMarkers.TASK_MARK_RE = /^(\s*(?:[-*+]|\d+\.)\s+\[)([ xX])(\])/;
  _TaskMarkers.FENCE_RE = /^(?:`{3,}|~{3,})/;
  _TaskMarkers.BQ_RE = /^\s*>[ \t]?/;
  var TaskMarkers = _TaskMarkers;
  function toggleNthTaskMarker(content, index2, checked) {
    return TaskMarkers.toggleNthTaskMarker(content, index2, checked);
  }

  // ../lib/content/task-checkboxes.ts
  var TaskCheckboxes = class {
    ...
    wireTaskCheckboxes(file, fullContent) {
      ...
      boxes.forEach((box, index2) => {
        ...
        box.addEventListener("change", () => {
          ...
          const newContent = toggleNthTaskMarker(docContent, index2, desired);
          if (newContent == null) {
            box.checked = !desired;
            return;
          }
  ...
  var taskCheckboxes = new TaskCheckboxes();

  // ../lib/content/backlinks.ts
  var backlinksIndex = null;
  function setBacklinksIndex(v) {
    backlinksIndex = v;
  }
  var backlinksLoading = null;
  function setBacklinksLoading(v) {
    backlinksLoading = v;
  }
  ...
  var _Editor = class _Editor {
    constructor() {
      ...
      this.wikilink = new WikilinkAutocomplete();
    ...
    async enterEditMode() {
      ...
      try {
        ...
        Dialogs.notifyError("cantLoadDoc", e.message);
        return;
      }
      setEditMode(true);
  ...
  var Pins = class {
    constructor() {
      this.section = document.getElementById("pinned-section");
      this.list = document.getElementById("pinned-list");
      this.btn = document.getElementById("btn-pin");
      this.btnIcon = document.getElementById("btn-pin-icon");
      this.pins = [];
      try {
  ...
  var historyPathEl = document.getElementById("history-path");
  var HistoryPanel = class {
    ...
    async showVersion(file, revisions, i) {
      ...
      wrap.innerHTML = markdown.render(stripFrontmatter(data.content || ""));
    }
  ...
  // ../lib/content/frames.ts
  var _Frames = class _Frames {
    constructor() {
  ...
  _Frames.FRAME_STYLE = "width:100%;height:calc(100vh - 150px);border:0;display:block;background:#0b0d13";
  // mammoth.js (DOCX → HTML) is ~640 KB: loaded on demand, never in the <head>, since most sessions
  // never open a .docx.
  _Frames.MAMMOTH_URL = "/vendor/mammoth.min.js";
  var Frames = _Frames;
  var frameRenderer = new Frames();

  // ../lib/content/notes/note-anchor.ts
  var _NoteAnchor = class _NoteAnchor {
  ...
  _NoteAnchor.CTX_LEN = 60;
  var NoteAnchor = _NoteAnchor;
  var noteAnchor = new NoteAnchor();

  // ../lib/content/notes/notes-index.ts
  ...
  // ../lib/content/notes/notes-store.ts
  var NotesStore = class {
    ...
    async saveNewNote(anchor, text) {
      ...
      try {
        ...
        Dialogs.notifyError("noteSaveFailed", e.message);
        return;
      }
    ...
    async saveEditNote(note, text) {
      ...
      try {
        ...
        Dialogs.notifyError("actionFailed", e.message);
        return;
      }
      notePopover.closeNotePop();
      this.refreshNotes();
  ...
  // ../lib/content/notes/note-popover.ts
  var NotePopover = class {
    constructor() {
      ...
      this.pendingButtonAnchor = null;
      this.pendingButtonRect = null;
      // Stateful islands: the floating "Note" button + the popover. Guaranteed by the viewer markup.
      this.noteAddBtn = document.getElementById("kb-note-add");
      this.notePop = document.getElementById("kb-note-pop");
  ...
  var notePopover = new NotePopover();

  // ../lib/content/notes/notes-panel.ts
  var noteContext = { notesForDoc: [] };
  var _NotesPanel = class _NotesPanel {
  ...
  var NotesPanel = _NotesPanel;
  var notesPanel = new NotesPanel();

  // ../lib/content/doc-renderer.ts
  ...
  var docRenderer = new DocRenderer();

  // ../lib/graph/graph-palette.ts
  var _GraphPalette = class _GraphPalette {
    constructor() {
    ...
    tagColor(tag) {
      ...
      let h2 = 0;
      for (let i = 0; i < tag.length; i++) h2 = h2 * 31 + tag.charCodeAt(i) >>> 0;
      return _GraphPalette.GRAPH_COLORS[h2 % _GraphPalette.GRAPH_COLORS.length];
    }
    ...
    hierColor(family, sub) {
      ...
      let h2 = 0;
      for (let i = 0; i < sub.length; i++) h2 = h2 * 31 + sub.charCodeAt(i) >>> 0;
      const hue = (baseHue + (h2 % 5 - 2) * 8 + 360) % 360;
      const light = 46 + Math.floor(h2 / 5) % 5 * 5;
      const sat = 60 + Math.floor(h2 / 25) % 3 * 9;
      return this.hslToHex(hue, sat, light);
    }
  };
  _GraphPalette.GRAPH_COLORS = [
    "#5db5e8",
    "#fbc678",
    "#a78bfa",
    "#34d399",
  ...
  var GraphPalette = _GraphPalette;

  // ../lib/graph/graph-layout.ts
  ...
  // ../lib/graph/mind-graph.ts
  var MindGraph = class {
    constructor() {
      this.overlay = document.getElementById("graph-overlay");
      this.canvas = document.getElementById("graph-canvas");
      this.tooltip = document.getElementById("graph-tooltip");
      this.stats = document.getElementById("graph-stats");
      // The injected pieces. canvas is declared above so the layout/renderer initializers can read it.
      this.palette = new GraphPalette();
      this.layout = new GraphLayout(this.canvas, this.palette);
      this.renderer = new GraphRenderer(this.canvas, this.palette);
      this.state = null;
      this.raf = null;
  ...
  // ../lib/graph/graph-boot.ts
  var mindGraph = new MindGraph();
  if (EMBED_MIND) {
  ...
  // ../lib/admin/settings/remotes-tab.ts
  var SettingsRemotes = class {
    constructor(ctx) {
      this.ctx = ctx;
      this.list = document.getElementById("settings-remotes-list");
      this.form = document.getElementById("settings-remote-form");
  ...
  // ../lib/admin/settings/groups-tab.ts
  var SettingsGroups = class {
    constructor(ctx) {
      this.ctx = ctx;
      this.wire();
    }
  ...
  // ../lib/admin/settings/profile-tab.ts
  var SettingsProfile = class {
    constructor(ctx) {
      this.ctx = ctx;
    }
    async load() {
  ...
  // ../lib/admin/settings/settings-panel.ts
  var SettingsPanel = class {
    constructor() {
      ...
      this.ctx = new SettingsContext();
      this.users = new SettingsUsers(this.ctx);
      this.tokens = new SettingsTokens(this.ctx);
      this.shares = new SettingsShares(this.ctx);
      this.nodes = new SettingsNodes(this.ctx);
      this.remotes = new SettingsRemotes(this.ctx);
      this.groups = new SettingsGroups(this.ctx);
      this.profile = new SettingsProfile(this.ctx);
  ...
  // ../lib/modals/quick-capture.ts
  var qcBtn = document.getElementById("quick-capture-btn");
  var qcBackdrop = document.getElementById("quick-capture-backdrop");
  var qcForm = document.getElementById("quick-capture-form");
  var qcTitle = document.getElementById("quick-capture-title");
  var qcBody = document.getElementById("quick-capture-body");
  var qcCancel = document.getElementById("quick-capture-cancel");
  var qcError = document.getElementById("quick-capture-error");
  var QuickCaptureModal = class extends Modal {
    constructor() {
      super(qcBackdrop);
  ...
  var quickCaptureModal = new QuickCaptureModal();

  // ../lib/modals/new-file-modal.ts
  var newFileBtn = document.getElementById("new-file-btn");
  var newFileBackdrop = document.getElementById("new-file-backdrop");
  var dirRenameBackdrop = document.getElementById("dir-rename-backdrop");
  function getAllDirs() {
  ...
  var NewFileModal = class extends Modal {
    constructor() {
      super(newFileBackdrop);
      this.form = document.getElementById("new-file-form");
      this.dir = document.getElementById("new-file-dir");
      this.name = document.getElementById("new-file-name");
      this.template = document.getElementById("new-file-template");
      this.visibility = document.getElementById("new-file-visibility");
      this.error = document.getElementById("new-file-error");
      this.cancel = document.getElementById("new-file-cancel");
  ...
  var newFileModal = new NewFileModal();
  var dirRenameModal = new DirRenameModal();
  window.Atlas = {
    version: 1,
    t,
    escapeHtml,
  ...
  // ../lib/modals/dialogs.ts
  var _Dialogs = class _Dialogs {
    static confirm(opts) {
  ...
  _Dialogs.backdrop = document.getElementById("confirm-backdrop");
  _Dialogs.titleEl = document.getElementById("confirm-title");
  _Dialogs.messageEl = document.getElementById("confirm-message");
  _Dialogs.okBtn = document.getElementById("confirm-ok");
  _Dialogs.cancelBtn = document.getElementById("confirm-cancel");
  var Dialogs = _Dialogs;
  var btnMore = document.getElementById("btn-more");
  var btnMoreMenu = document.getElementById("btn-more-menu");
  var renameBackdrop = document.getElementById("rename-backdrop");
  var renameForm = document.getElementById("rename-form");
  var renameTitle = document.getElementById("rename-title");
  var renameDir = document.getElementById("rename-dir");
  var renameDirWrap = document.getElementById("rename-dir-wrap");
  var renameName = document.getElementById("rename-name");
  ...
  var renameModal = new RenameModal();

  // ../lib/content/content-tree.ts
  ...
  var WL_TARGET_EXTS = [".md", ".html", ".pdf", ".docx"];
  var _wlMaps = null;
  function setWlMaps(v) {
    _wlMaps = v;
  }
  function wlMaps() {
  ...
  var _ContentTree = class _ContentTree {
    constructor() {
      ...
      this.openDirs = /* @__PURE__ */ new Set();
    }
    iconFor(ext) {
      ...
      if (!isRemote) {
        btnChildren.push(
          h("span", { class: "dir-share-btn tree-action-btn tree-action-btn--share", title: t("shareAsNode"), onClick: (e) => {
            e.stopPropagation();
            settingsPanel.openPublish(childPath);
      ...
      if (fileActionable) {
        aChildren.push(
          ...
          h("span", { class: "file-share-btn tree-action-btn tree-action-btn--share", title: t("shareAsNode"), onClick: (e) => {
            e.preventDefault();
            e.stopPropagation();
            settingsPanel.openPublish(child.path);
          } }, raw(_ContentTree.LINK_ICON))
        );
      }
      const props = {
    ...
    async decorateRemoteOrigins() {
      ...
      for (const r of remotes) {
        const host2 = (r.url || "").replace(/^https?:\/\//, "").replace(/\/.*$/, "");
        if (!host2) continue;
  ...
  // ../lib/graph/todo-surface.ts
  var todoWidget = document.getElementById("todo-widget");
  var todoHeader = document.getElementById("todo-header");
  var todoBody = document.getElementById("todo-body");
  var todoChevron = document.getElementById("todo-chevron");
  var todoList = document.getElementById("todo-list");
  var todoForm = document.getElementById("todo-form");
  var todoInput = document.getElementById("todo-input");
  var todoCount = document.getElementById("todo-count");
  var todoBubbleCount = document.getElementById("todo-bubble-count");
  ...
  var showDoneTodos = localStorage.getItem("todo-show-done") === "1";
  function setShowDoneTodos(v) {
    showDoneTodos = v;
  }
  var TODO_CATEGORIES = __TODO_CATEGORIES_JSON__;
  var TODO_CATS = TODO_CATEGORIES.map((c) => c.cat);
  var TODO_FILTER_LABELS = Object.fromEntries(
    TODO_CATEGORIES.map((c) => [c.cat, c.label])
  );
  function tcat(td) {
    return TODO_CATS.includes(td.cat) ? td.cat : TODO_CATS[0];
  }
  var todoFilter = localStorage.getItem("todo-filter");
  function setTodoFilter(v) {
    todoFilter = v;
  }
  ...
  // ../lib/editor/search.ts
  var miniSearch = null;
  var searchInitPromise = null;
  function setMiniSearch(v) {
    miniSearch = v;
  }
  function setSearchInitPromise(v) {
    searchInitPromise = v;
  }
  var _Search = class _Search {
    constructor() {
      ...
      this.treeHeaderEl = document.getElementById("tree-header");
    ...
    async getSearchHits(q) {
      ...
      return hits.map((h2) => ({ path: h2.path, snippet: h2.snippet || "" }));
    }
  ...
  _Search.SEARCH_FIELDS = ["name", "path", "content"];
  _Search.SEARCH_STORE = ["name", "path", "preview"];
  var Search = _Search;
  ...
  // ../lib/graph/command-palette.ts
  var _CommandPalette = class _CommandPalette {
    constructor() {
  ...
  var CommandPalette = _CommandPalette;
  var commandPalette = new CommandPalette();

  // ../lib/graph/tasks-overlay.ts
  var TasksOverlay = class {
    // kept for the "show done" toggle re-render
    constructor() {
      this.overlay = document.getElementById("tasks-overlay");
      this.list = document.getElementById("tasks-list");
      this.stats = document.getElementById("tasks-stats");
      this.showDoneBox = document.getElementById("tasks-show-done");
      this.index = [];
  ...
  var tasksOverlay = new TasksOverlay();

  // ../lib/graph/keyboard-router.ts
  var KeyboardRouter = class {
    constructor() {
  ...
  var keyboardRouter = new KeyboardRouter();
  ...
  // ../lib/modals/more-actions-menu.ts
  var MoreActionsMenu = class {
    constructor() {
  ...
  var moreActionsMenu = new MoreActionsMenu();

  // ../lib/admin/access-dialog.ts
  var AccessDialog = class {
    constructor() {
      ...
      this.backdrop = document.getElementById("acl-backdrop");
      this.pathEl = document.getElementById("acl-path");
      this.statusEl = document.getElementById("acl-status");
      this.grantsEl = document.getElementById("acl-grants");
      this.manageEl = document.getElementById("acl-manage");
      this.form = document.getElementById("acl-grant-form");
      this.kindSel = document.getElementById("acl-kind");
      this.valueInp = document.getElementById("acl-value");
      this.levelSel = document.getElementById("acl-level");
      this.errEl = document.getElementById("acl-error");
      // ---- state ----
      this.cur = null;
      this.dir = null;
      ...
      this.aclCb = AtlasCombobox(this.valueInp, {
  ...
  // ../lib/admin/activity/activity-icons.ts
  var ActivityIcons = class {
  };
  ...
  ActivityIcons.ORRERY_CAP = 18;
  // aggregated entries (distinct doc-activities), not raw commits
  ActivityIcons.JOURNAL_PREVIEW = 8;
  ...
  // ../lib/admin/activity/activity-model.ts
  var ActivityModel = class {
  ...
  // ../lib/admin/activity/activity-journal.ts
  var ActivityJournal = class {
    constructor(ctx) {
      this.ctx = ctx;
    }
    row(e) {
  ...
  // ../lib/admin/activity/activity-orrery.ts
  var ActivityOrrery = class {
  ...
  // ../lib/admin/activity/activity-health.ts
  var ActivityHealth = class {
    constructor(ctx) {
      this.ctx = ctx;
      this.health = null;
      this.healthExpanded = false;
      this.candExpanded = false;
  ...
  // ../lib/admin/activity/activity-card.ts
  var ActivityCard = class {
    constructor() {
      // ---- state ----
      this.items = null;
      this.aiOnly = false;
  ...
  // ../lib/admin/activity/activity-boot.ts
  ...
  _Inbox.IDOC = "M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z";
  _Inbox.ILINK = "M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244";
  _Inbox.ICHECK = "M4.5 12.75l6 6 9-13.5";
  _Inbox.ITRASH = "M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.1 48.1 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.1 48.1 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.96 51.96 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.67 48.67 0 0 0-7.5 0";
  _Inbox.ISNOOZE = "M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5";
  _Inbox.IPENCIL = "m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.931-8.931Zm0 0L19.5 7.125";
  _Inbox.SKEY = { keep: "kept", trash: "trashed", snooze: "snoozed" };
  var Inbox = _Inbox;
  ...
  var Boot = class {
    constructor() {
      ...
      this.swUpdatePending = false;
      this.swReg = null;
      this.hadController = false;
    }
    start() {
      if (isServerMode) {
        this.serverBoot();
