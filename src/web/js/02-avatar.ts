...
//
// Seeded-deterministic: the ORDER in which the constructor pulls from its Rng decides
// a person's avatar, so it is fixed. The render methods only turn that drawn state
// into SVG — no randomness, reorder-proof.
(function (root: any) {
  'use strict';

  // A seeded pseudo-random generator (mulberry32) hashed from an identity string. Holds
  // its own evolving state, so an Avatar draws its whole design from one Rng in order.
  class Rng {
  ...
  // Public global API (consumed by the still-.js modules as bare globals; the IIFE keeps
  // Rng/Avatar and their helpers out of the shared bundle scope).
  root.constellationSvg = (identity: string, size?: number) => new Avatar(identity, size).render();
