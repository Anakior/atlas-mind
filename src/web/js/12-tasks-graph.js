  ...
  for (let s = 0; s < 3; s++) {
    ...
    for (let r = 0, n = range(2, 4); r < n; r++) {
      rows.push(
        ...
        + '<span class="skeleton" style="height:0.95rem;width:' + range(45, 90) + '%;margin-top:5px;"></span>'
        + '</div>');
    }
    sections.push(
      '<div style="margin-bottom:1.75rem;">'
      + '<div class="skeleton" style="height:0.7rem;width:' + range(22, 42) + '%;border-radius:4px;margin-bottom:0.6rem;"></div>'
      + rows.join('') + '</div>');
  }
...
function showTasksLoading() {
  tasksStats.innerHTML = '<span class="skeleton" style="display:inline-block;height:0.7rem;width:9rem;border-radius:4px;vertical-align:middle;"></span>';
  ...
  if (!(li && want && _normTask(li.textContent).includes(want))) {
    ...
    if (want) {
      for (const b of boxes) {
        ...
        if (candidate && _normTask(candidate.textContent).includes(want)) { li = candidate; break; }
      }
    }
  }
  if (!li) { highlightFirstMatch(contentEl, task.text); return; }
...
function graphSimStep(st) {
  ...
  const REP = 13000, SPRING = 0.02, REST = 120, CENTER = 0.0034;
...
function graphDraw(st) {
  ...
  for (const e of edges) {
    ...
    if (e.kind === 'link') {
      ...
      ctx.beginPath(); ctx.moveTo(ax, ay); ctx.quadraticCurveTo(e._cx, e._cy, bx, by); ctx.stroke();
    } else {
  ...
  for (const e of edges) {
    ...
    const ax = SX(e.s), ay = SY(e.s), bx = SX(e.t), by = SY(e.t);
    const tt = ((now * 0.00016) + (pi++ * 0.1379)) % 1, u = 1 - tt;
    ...
    ctx.fillStyle = 'rgba(200,188,250,0.9)'; ctx.fill();
    ctx.globalAlpha = 1;
  }
  ctx.restore();
  ...
  for (const n of nodes) {
    ...
    const breath = n.recent ? 1 + 0.12 * Math.sin(now * 0.004 + (n._ph || (n._ph = (Math.abs(n.x) + Math.abs(n.y)) % 6.28))) : 1;
  ...
  for (const n of nodes) {
    ...
    else if (n.doctype && n.doctype !== '.md' && !dim) { ctx.lineWidth = 1.5; ctx.strokeStyle = 'rgba(255,255,255,0.72)'; ctx.stroke(); }
