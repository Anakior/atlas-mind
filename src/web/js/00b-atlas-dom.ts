...
// of them, so the whole model can be read top to bottom and audited. Concatenated into the
// shared bundle scope (no import/export); only the public API (h, raw, render, createApp,
// Show) is exposed as a global, declared in globals.d.ts for tsc. The ~15 internal helpers
// stay private to this IIFE.
(function (root: any) {
  'use strict';
  ...
  function h(tag: string, props?: Record<string, any> | null, ...children: Child[]): VNode {
    const p = props || {};
  ...
  function raw(html: string): VNode {
    return { tag: '#raw', props: {}, children: [], text: html };
  }

  function Show(cond: any, view: () => Child): Child {
  ...
  function render(next: Child, container: Element): void {
  ...
  function createApp(container: Element, view: (state?: any) => Child) {
    return {
