...
// encodes a string to a module matrix, no DOM (it stays inside the IIFE). The DOM bridge renderQrCanvas
// (matrix → <canvas>) is a free bundle global below; the 2FA enrollment wiring that calls it lives in
// 18-totp-modal.ts. Public surface: the QrCode class (`new QrCode(text).matrix`) + renderQrCanvas.
//
...
(function (root: any) {
  ...
  // Public global API: QrCode on the shared scope, consumed by renderQrCanvas below (and the 2FA flow)
  // as a bare global; the IIFE keeps Gf256/QrCode's internals out of the shared bundle scope.
  root.QrCode = QrCode;
...
// reading QrCode off the shared scope the IIFE above assigned.
