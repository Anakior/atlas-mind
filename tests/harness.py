...
- Each AtlasServer creates a self-contained temporary directory (copy of src/,
  viewer/ and templates/ — the engine's document skeletons —, a test content/
  mind, an initialized git repo, dist/ generated by the build) then launches the
  engine's server (python -m server) in a subprocess on an ephemeral port.
...
- The engine package (server/, build/, config.py, store.py) MUST live in
  <tmp>/src/ and the content in <tmp>/content/ — the harness copies src/ there.
- The server does NOT build at boot in local mode: the harness runs the engine's
  build itself (python -m build) to produce dist/index.html (otherwise GET /
  falls back to the listing of the content/ directory).
...
WEB_DIR = SRC_DIR / "viewer"         # engine assets now bundled inside the package
...
class AtlasServer:
    ...
    def _populate(self) -> None:
        ...
        # bundle/css — the SAME artifact, far faster. Tests never edit viewer/lib sources.
        ignore = shutil.ignore_patterns("__pycache__", "*.pyc", "node_modules")
        # viewer/ and templates/ now live INSIDE src/ (the package), so copying src/
    ...
    def build(self) -> subprocess.CompletedProcess:
        """Builds the mind's viewer via `python -m build` (generates dist/index.html
